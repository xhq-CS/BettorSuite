...
groupsRouter.get("/:id/messages",async(req,res)=>{const groupId=Number(req.params.id),userId=uid(req);if(!await requireMember(groupId,userId))return void res.status(403).json({error:"Join this group to view messages"});const rows=await db.select({id:groupMessagesTable.id,senderId:groupMessagesTable.senderId,senderUsername:usersTable.username,content:groupMessagesTable.content,betShare:groupMessagesTable.betShare,dailyCardId:groupMessagesTable.dailyCardId,createdAt:groupMessagesTable.createdAt,editedAt:groupMessagesTable.editedAt}).from(groupMessagesTable).innerJoin(usersTable,eq(groupMessagesTable.senderId,usersTable.id)).where(eq(groupMessagesTable.groupId,groupId)).orderBy(groupMessagesTable.createdAt).limit(200);res.json(await Promise.all(rows.map(async m=>({...m,betShare:m.betShare??null,dailyCard:await getDailyCard(m.dailyCardId),createdAt:m.createdAt.toISOString(),editedAt:m.editedAt?.toISOString()??null}))))});
