...
export default function Groups({ embedded = false }: { embedded?: boolean }) {
  ...
  const create = useMutation({ mutationFn:()=>api<Group>("/groups",{method:"POST",body:JSON.stringify({name,description})}), onSuccess:g=>{queryClient.invalidateQueries({queryKey:["groups"]}); navigate(`/groups/${g.id}?from=community`);} });
